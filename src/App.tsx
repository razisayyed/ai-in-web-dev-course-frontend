...
export function App() {
  ...
  // State for the "upload a receipt" dialog
  ...
  async function saveExpenseFromText() {
    const url = "http://localhost:8000/expenses/from-text"
    const response = await fetch(url, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
  ...
  // // Send a receipt picture to the backend, the AI reads the expenses from it
  // async function saveExpenseFromImage() {
  //   // A file is sent as FormData, not as JSON.
  //   // "image" must match the parameter name in the backend.
  //   const formData = new FormData()
  //   formData.append("image", receiptFile)
  //   const response = await fetch("http://localhost:8000/expenses/from-image", {
  //     method: "POST",
  //     // No "Content-Type" header here on purpose:
  //     // the browser adds it for us when we send FormData.
  //     body: formData,
  //   })
  // }
    ...
    <div className="flex min-h-svh p-6">
      <div className="flex max-w-md min-w-0 flex-col gap-4 text-sm leading-loose">
        <div className="flex gap-2">
          ...
          {/* Free user text dialog */}
          <Dialog open={isTextDialogOpen} onOpenChange={setIsTextDialogOpen}>
            <DialogTrigger
              render={<Button>Add from text</Button>}
            ></DialogTrigger>
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Expenses Form</DialogTitle>
                <DialogDescription>
                  Fill in the textarea below a free text about your expenses and
                  the system will convert it to structured data and add it to
              ...
              <DialogFooter>
                <Button type="submit" onClick={saveExpenseFromText}>
                  Save changes
                </Button>
              </DialogFooter>
            </DialogContent>
